...
class SearchResultsPage(QWidget):
    ...
    def __init__(self, request_id=None, parent=None):
        ...
        self.radio_buttons = []  # Хранилище радио-кнопок
    ...
    def _clear_results(self):
        ...
        self.radio_buttons.clear()  # Очищаем список радио-кнопок
    ...
    def create_product_card(self, product_data):
        ...
        for i, variant in enumerate(product_data['variants']):
            ...
            # Радиокнопка выбора
            radio = QRadioButton()
            radio.setStyleSheet(f"QRadioButton {{ margin: 10px; }}")
            ...
            # Подключаем к пересчету суммы
            radio.toggled.connect(self._calculate_total)
    ...
    def _calculate_total(self):
        """Подсчитать итоговую сумму выбранных товаров"""
        total = 0.0
        
        # Проходим по всем карточкам товаров
        for i in range(self.scroll_layout.count()):
            widget = self.scroll_layout.itemAt(i).widget()
            if isinstance(widget, QFrame) and widget.objectName() == "formCard":
                table = widget.findChild(QTableWidget)
                if table:
                    for row in range(table.rowCount()):
                        radio_widget = table.cellWidget(row, 0)
                        if radio_widget:
