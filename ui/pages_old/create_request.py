...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                             QPushButton, QFrame, QSpinBox, QLineEdit,
                             QDateEdit, QComboBox, QApplication)
from PyQt6.QtCore import Qt, QDate
...
class CreateRequestPage(QWidget):
    def __init__(self, icons_path, parent=None):
        ...
        # Создаем только контент этой страницы (без сайдбара и настроек окна)
        self.create_widgets()
        self.apply_styles()

    def create_widgets(self):
        """Создание элементов интерфейса СТРАНИЦЫ создания заявки"""
        ...
        # ✅ Поле 1: Количество студентов (сохраняем ссылку для очистки)
        self.students_spinbox = self.create_students_input()
        students_row = self.create_form_row(
            "group.png",
            "Количество студентов",
            self.students_spinbox,
            tooltip_text="Общее количество студентов, которые будут использовать оборудование в течение года"
        )
        form_layout.addLayout(students_row)
        form_layout.addWidget(self.create_separator())
        ...
        content_layout.addWidget(form_card)
        content_layout.addStretch()  # Растягиваем, чтобы форма была сверху
    ...
    def create_form_row(self, icon_file, label_text, input_widget, tooltip_text=""):
        ...
        if isinstance(input_widget, (QHBoxLayout, QVBoxLayout)):
            right_layout.addLayout(input_widget)
        else:
            right_layout.addWidget(input_widget)
    ...
    def create_price_input(self):
        """Создание полей мин/макс цены (возвращает сами виджеты)"""
    ...
    def create_date_input(self):
        date_edit = QDateEdit()
        date_edit.setDate(QDate(2024, 6, 15))  # Можно заменить на QDate.currentDate()
        date_edit.setDisplayFormat("dd.MM.yyyy")
        date_edit.setFont(QFont("Segoe UI", 12))
        date_edit.setFixedHeight(45)
        date_edit.setFixedWidth(300)
    ...
    def on_next(self):
        """Обработчик кнопки 'Далее'"""
        print("Переход к следующему шагу")
        # Здесь можно добавить валидацию и переход на страницу результатов
        # self.navigate_to("Результаты")

    def navigate_to(self, page_name):
        """Вспомогательный метод для переключения страниц изнутри виджета"""
        parent = self.parent()
...
# Блок для автономного тестирования этой страницы (не влияет на основную программу)
if __name__ == "__main__":
    app = QApplication(sys.argv)
    app.setStyle("Fusion")
    app.setFont(QFont("Segoe UI", 10))
