...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                             QPushButton, QFrame, QGridLayout, QSizePolicy, QDialog, QApplication)
from PyQt6.QtCore import Qt
from PyQt6.QtGui import QFont, QPixmap, QIcon

from ui.styles import COMMON_STYLES, CARD_STYLES, EXIT_CARD_STYLES
...
class MainPage(QWidget):
    ...
    def create_widgets(self):
        """Создание элементов интерфейса ГЛАВНОЙ страницы"""
        ...
        cards_data = [
            ...
            ("product.png", "Справочник товаров", "Просмотрите или редактируйте актуальные товары", "#8B5CF6",
             "#FFFACD"),
            ("door.png", "Выход из приложения", "Завершить работу и выйти из системы", "#EF4444", "#FEE2E2"),
        ]
        ...
        layout.addWidget(cards_widget)
        layout.addStretch()  # Растягиваем, чтобы карточки были сверху

        # 3. Exit Card в правом нижнем углу (если нужна на каждой странице)
        # Примечание: В идеальной архитектуре QStackedWidget exit_card лучше
        # разместить один раз в app_window.py, но можно и здесь.
    ...
    def navigate_to(self, page_name):
        ...
        # Ищем главное окно (AppWindow) среди родителей
        parent = self.parent()
    ...
    def show_exit_confirmation(self):
        ...
        # Берем иконку из главного окна, если есть
        parent = self.parent()
        while parent is not None:
        ...
        cancel_btn.setFixedSize(150, 45)
        cancel_btn.setStyleSheet(
            "QPushButton { background-color: #F3F4F6; color: #374151; border: 1px solid #D1D5DB; border-radius: 8px; } QPushButton:hover { background-color: #E5E7EB; }")
        cancel_btn.clicked.connect(dialog.reject)

        exit_btn = QPushButton("Выйти")
        exit_btn.setFixedSize(150, 45)
        exit_btn.setStyleSheet(
            "QPushButton { background-color: #EF4444; color: white; border: none; border-radius: 8px; font-weight: bold; } QPushButton:hover { background-color: #DC2626; }")
        exit_btn.clicked.connect(lambda: self.confirm_exit(dialog))
    ...
    def apply_styles(self):
        ...
        self.setStyleSheet(COMMON_STYLES + CARD_STYLES + EXIT_CARD_STYLES)
